...
type Extender interface {
	...
	Extend(ctx context.Context,
		...
		ingress, intfGroup, egress uint16, algId uint32, algHash []byte, emiroExt bool,
		peers []uint16) error
}
...
func (s *DefaultExtender) Extend(
	ctx context.Context,
	pseg *seg.PathSegment,
	ingress, intfGroup, egress uint16, algId uint32, algHash []byte, irecExtension bool,
	peers []uint16,
) error {
	...
	if static := s.StaticInfo(); static != nil {
		//TODO(jvb): static extension should be arbitrarily extendable.
